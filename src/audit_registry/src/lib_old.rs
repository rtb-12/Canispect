...
type AuditStorageType = StableBTreeMap<String, AuditRecord, Memory>;
...
pub fn update_audit_record(
    ...
    AUDIT_STORAGE.with(|storage| {
        ...
        if let Some(mut record) = storage.get(&audit_id) {
...
pub fn get_audit_record(audit_id: String) -> Option<AuditRecord> {
    AUDIT_STORAGE.with(|storage| storage.borrow().get(&audit_id))
}
...
#[pre_upgrade]
fn pre_upgrade() {
    // Stable memory is handled automatically by StableBTreeMap
}
